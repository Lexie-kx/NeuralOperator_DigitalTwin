...
## 🎯 项目摘要 (Abstract)
本项目构建了一套从 2D 基础流场到复杂工业级物理约束流场的全流程代理模型体系。通过 **POD-DeepONet** 架构，实现了**高分辨率网格流场的高维压缩**与毫秒级非线性映射。项目核心突破在于引入了基于**有限差分预计算雅可比矩阵**的 PINO 约束，在不增加显存开销的前提下，强制模型输出遵守质量守恒定律。
